...
from quant_search.quant_layer import QuantModule, Union
from quant_search.quant_model import QuantModel
from quant_search.quant_block import BaseQuantBlock, QuantResBlock, QuantAttentionBlock


def save_inp_oup_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor, cali_ts_list: torch.tensor,
    ...
    def __call__(self, model_input, cali_ts):
        ...
        with torch.no_grad():
            ...
            if self.asym:
                ...
                self.model.set_quant_state(weight_quant=False, act_quant=self.act_quant)
                try:
                    _ = self.model(model_input.to(self.device), cali_ts.to(self.device))
                except StopForwardException:
                    pass
