...
class UniformAffineQuantizer(nn.Module):
    ...
    def __init__(self, n_bits: int = 8, symmetric: bool = False, channel_wise: bool = False, scale_method: str = 'max',
                 leaf_param: bool = False, softmax_inited=False, always_zero: bool = False):
        ...
        self.softmax_inited = softmax_inited
        self.running_max = None
        self.running_min = None
        self.calibrated = False
        self.always_zero = always_zero
        self.thd_pos = 2 ** self.n_bits if not self.sym else 2 ** (self.n_bits - 1) - 1
        self.is_training = False
        self.zp_fixed = False
        self.train_flag = False
        self.max_value = []
        self.min_value = []
        self.zp_update = False
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.leaf_param and not self.calibrated:
            ...
            self.max_value.append(x.max().cpu().detach().numpy())
            self.min_value.append(x.min().cpu().detach().numpy())
        ...
        # drop
        if self.is_training:
            x_ans = torch.where(torch.rand_like(x) < 1.0, x_dequant, x)
        else:
            x_ans = x_dequant
        return x_ans
    ...
    def set_calibrated(self):
        ...
        # zero_point = torch.tensor(zero_point).type_as(self.running_max)
    ...
    def get_clip_value(self, x):
        x_max = x.max()
        x_min = x.min()
        best_score = 1e+10
        best_clip_value = x_max
        for i in range(80):
            new_max = x_max * (1.0 - (i * 0.01))
            new_min = x_min
            x_q = self.quantize(x, new_max, new_min)
            ...
            if score < best_score:
                best_score = score
                best_clip_value = new_max
            
        return best_clip_value
    ...
    def update_zp(self):
        ...
        # self.zero_point.data = torch.tensor(round(-self.running_min.item() / self.delta.item()))
    ...
    def forward(self, input: torch.Tensor, split: int = 0):
        ...
        if not self.disable_act_quant and self.use_act_quant:
            ...
            if self.split != 0:
                ...
                # if self.name == "output_blocks.11.0.skip_connection":
                #     x_max0 = input_0.max().cpu().detach().numpy()
                #     x_min0 = input_0.min().cpu().detach().numpy()
                #     f = "./out/range/linear_max_0.txt"
                #     with open(f, "a") as file:
                #         file.write(str(x_max0) + "\n")
                #     f = "./out/range/linear_min_0.txt"
                #     with open(f, "a") as file:
                #         file.write(str(x_min0) + "\n")

                #     x_max1 = input_1.max().cpu().detach().numpy()
                #     x_min1 = input_1.min().cpu().detach().numpy()
                #     f = "./out/range/linear_max_1.txt"
                #     with open(f, "a") as file:
                #         file.write(str(x_max1) + "\n")
                #     f = "./out/range/linear_min_1.txt"
                #     with open(f, "a") as file:
                #         file.write(str(x_min1) + "\n")
