...
from quant_qdrop_split.quant_layer import QuantModule, Union
from quant_qdrop_split.quant_model import QuantModel
from quant_qdrop_split.quant_block import BaseQuantBlock, QuantResBlock, QuantAttentionBlock, QuantTimestepEmbedSequential


def save_inp_oup_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock, QuantTimestepEmbedSequential], cali_data: torch.Tensor, cali_ts_list: torch.tensor,
                      asym: bool = False, act_quant: bool = False, batch_size: int = 32, keep_gpu: bool = True, only_out: bool = False):
    """
    ...
    if isinstance(layer, (QuantModule, QuantAttentionBlock)):
        for i in range(int(cali_data.shape[0] / batch_size)):
            cur_inp, cur_out, cur_sym = get_inp_out(cali_data[i * batch_size:(i + 1) * batch_size], cali_ts_list[i * batch_size:(i + 1) * batch_size])
        ...
        # cached_syms = torch.cat([x[2] for x in cached_batches])
        # torch.cuda.empty_cache()
        # if keep_gpu:
        #     cached_inps = cached_inps.to(device)
        #     cached_outs = cached_outs.to(device)
        #     cached_syms = cached_syms.to(device)
        # return cached_inps, cached_outs, cached_syms
    ...
    elif isinstance(layer, QuantResBlock):
        for i in range(int(cali_data.shape[0] / batch_size)):
            cur_inp, cur_emb, cur_out, sym_inp, sym_emb = get_inp_out(cali_data[i * batch_size:(i + 1) * batch_size], cali_ts_list[i * batch_size:(i + 1) * batch_size])
            cached_batches.append((cur_inp.cpu(), cur_emb.cpu(), cur_out.cpu(), sym_inp.cpu(), sym_emb.cpu()))

        # cached_inps = torch.cat([x[0] for x in cached_batches])
        # cached_embs = torch.cat([x[1] for x in cached_batches])
        # cached_outs = torch.cat([x[2] for x in cached_batches])
        # input_syms = torch.cat([x[3] for x in cached_batches])
        # emb_syms = torch.cat([x[4] for x in cached_batches])
        # torch.cuda.empty_cache()
        # if keep_gpu:
        #     cached_inps = cached_inps.to(device)
        #     cached_embs = cached_embs.to(device)
        #     cached_outs = cached_outs.to(device)
        #     input_syms = input_syms.to(device)
        #     emb_syms = emb_syms.to(device)
        # return cached_inps, cached_embs, cached_outs, input_syms, emb_syms
        ...
        if not only_out:
            if keep_gpu:
                ...
                cached_embs = cached_embs.to(device)
                cached_outs = cached_outs.to(device)
            return cached_inps, cached_embs, cached_outs
        else:
            if keep_gpu:
                cached_outs = cached_outs.to(device)
            return cached_outs
    
    elif isinstance(layer, QuantTimestepEmbedSequential):
        for i in range(int(cali_data.shape[0] / batch_size)):
            cur_inp, cur_emb, cur_out, sym_inp, sym_emb = get_inp_out(cali_data[i * batch_size:(i + 1) * batch_size], cali_ts_list[i * batch_size:(i + 1) * batch_size])
            cached_batches.append((cur_inp.cpu(), cur_emb.cpu(), cur_out.cpu(), sym_inp.cpu(), sym_emb.cpu()))
...
class GetLayerInpOut:
    def __init__(self, model: QuantModel, layer: Union[QuantModule, BaseQuantBlock, QuantTimestepEmbedSequential],
    ...
    def __call__(self, model_input, cali_ts):
        self.model.eval()
        ...
        with torch.no_grad():
            try:
                _ = self.model(model_input.to(self.device), cali_ts.to(self.device))
            except StopForwardException:
                pass
            
            if isinstance(self.layer, (QuantModule, QuantAttentionBlock)):
                input_sym = self.data_saver.input_store[0].detach()
            elif isinstance(self.layer, (QuantResBlock, QuantTimestepEmbedSequential)):
                input_sym = self.data_saver.input_store[0].detach()
                emb_sym = self.data_saver.input_store[1].detach()
        ...
        elif isinstance(self.layer, QuantResBlock):
            return self.data_saver.input_store[0].detach(), self.data_saver.input_store[1].detach(), self.data_saver.output_store.detach(), input_sym, emb_sym
        elif isinstance(self.layer, QuantTimestepEmbedSequential):
            return self.data_saver.input_store[0].detach(), self.data_saver.input_store[1].detach(), self.data_saver.output_store.detach(), input_sym, emb_sym
