...
import numpy as np
from quant_qdrop_split.quant_layer import QuantModule, StraightThrough, lp_loss
from quant_qdrop_split.quant_model import QuantModel
from quant_qdrop_split.block_recon import LinearTempDecay
...
def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor, cali_ts_list: torch.tensor,
    ...
    cached_inps, cached_outs = save_inp_oup_data(model, layer, cali_data, cali_ts_list, asym, act_quant, batch_size)
    ...
    # a_para += [layer.bias]
    if layer.act_quantizer.zp_update == True:
