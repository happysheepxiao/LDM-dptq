...
import numpy as np
from quant_qdrop_split.quant_layer import QuantModule, StraightThrough, lp_loss, UniformAffineQuantizer
from quant_qdrop_split.quant_model import QuantModel
from quant_qdrop_split.quant_block import BaseQuantBlock, QuantResBlock, QuantAttentionBlock, QuantQKMatMul, QuantSMVMatMul
from quant_qdrop_split.adaptive_rounding import AdaRoundQuantizer
from quant_qdrop_split.data_utils import save_grad_data, save_inp_oup_data
...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor, cali_ts_list: torch.tensor,
    ...
    if isinstance(block, QuantResBlock):
        ...
        cached_inps, cached_embs, cached_outs = save_inp_oup_data(model, block, cali_data, cali_ts_list, asym, act_quant, batch_size)
        ...
        cached_inps, cached_outs = save_inp_oup_data(model, block, cali_data, cali_ts_list, asym, act_quant, batch_size)
        # cached_outs_dis = save_inp_oup_data(model, block, dis_data, dis_ts, asym, act_quant, batch_size, only_out=True)
    ...
    for name, module in block.named_modules():
        ...
        if isinstance(module, QuantModule):
            if module.split == 0:
                ...
                # a_para += [module.bias]
                if module.act_quantizer.zp_update == True:
                    a_para += [module.act_quantizer.zero_point]
                ...
                if module.act_quantizer.zp_update == True:
                    a_para += [module.act_quantizer.zero_point]
                if module.act_quantizer_0.zp_update == True:
                    a_para += [module.act_quantizer_0.zero_point]
    ...
    if isinstance(block, QuantAttentionBlock):
        ...
        # i = 0
        # while hasattr(block.attention.smv_matmul, 'act_quantizer_w_' + str(i)):
        #     a_para += [getattr(block.attention.smv_matmul, 'act_quantizer_w_' + str(i)).delta]
        #     i += 1
    ...
    if isinstance(block, QuantResBlock):
        ...
        for i in range(iters):
            ...
            # err = loss_func(out_quant, cur_out, cur_next)
